...
export class AnalyseurCondition {
  ...
  /**
   * Décomposer la condition selon la priorité des oppérateurs.
  ...
  public static decomposerConditionBrute(conditionBrute: string): ConditionDecomposee {
    ...
    if (conditionBrute && AnalyseurCondition.parenthesesValides(conditionBrute)) {
      conditionDecomposee = this.decomposerMorceauConditionBrute(conditionBrute, LienCondition.aucun, []);
    }
  ...
  /**
   * Suite décomposer la condition selon la priorité des oppérateurs.
   * @param conditionSource 
   */
  private static decomposerMorceauConditionBrute(morceauConditionBrute: string, lien: LienCondition, sousConditionsBrutes: string[]): ConditionDecomposee {
    ...
    if (resultatRefSousCond) {
      ...
      // );
      retVal = this.decomposerMorceauConditionBrute(sousConditionsBrutes[indexSousCond], lien, sousConditionsBrutes);
      ...
      if (morceauxConditionPrincipale.length > 1) {
        ...
        for (let indexMorceau = 0; indexMorceau < morceauxConditionPrincipale.length; indexMorceau++) {
          const newSousCondition = this.decomposerMorceauConditionBrute(morceauxConditionPrincipale[indexMorceau], prochainLien, sousConditionsBrutes);
          if (!newSousCondition) {
